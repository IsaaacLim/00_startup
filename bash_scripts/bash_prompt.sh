...
# Comfirmation prompt
read -p "Override existing ~/.bash_prompt file? [y/n] " -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]
then
	# Creates a BASH command prompt mod script into ~/.bash_prompt
...
' > ~/.bash_prompt 
	...
	echo "	~/.bash_prompt.sh has been overwritten"
else
	echo "	~/.bash_prompt.sh wasn't modified..."
	echo "	check bash prompt settings from /bash_scripts/bash_prompt.sh"
fi
